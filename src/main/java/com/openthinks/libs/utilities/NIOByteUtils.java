...
public final class NIOByteUtils {
	...
	/**
	 * convert each byte in array to unsigned HEX string 
	 * @param vals byte array
	 * @return unsigned HEX string
	 */
	public static String toHexString(byte[] vals) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0, j = vals.length; i < j; i++) {
			int unsigned = Byte.toUnsignedInt(vals[i]);
	...
	/**
	 * 
	 * copyOf:copy of given byte array with fix length,and fill left space with
	 * 0x00. <br>
	 * 
	 * @param src
	 *            byte array need copy
	 * @param length
	 *            new byte array fix length
	 * @param fillLeft
	 *            boolean fill 0x00
	 * @return byte array
	 */
