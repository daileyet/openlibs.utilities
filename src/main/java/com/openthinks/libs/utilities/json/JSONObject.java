...
public final class JSONObject extends LinkedHashMap<String, Object> implements Serializable {
	...
	/**
	 * get property value by property name from this JSON object, and cast assigned
	 * type <BR>
	 * 
	 * @param <T>
	 *            property type
	...
	public <T> T getProperty(String propertyName, Class<T> propertyType) {
		...
		if (runtimClassType != JSONArray.class && propertyType == JSONArray.class
			...
			try {
				return Converter.source(value).convertToSingle(propertyType);
			} catch (Exception e) {
				return (T)value;
			}
		}
	...
	public JSONElement getPropertyAsElement(String propertyName) {
		return new JSONElement(getProperty(propertyName));
	}
	...
	/**
	 * add new key and value entry into JSON object, and the value type is
	 * {@link JSONArray}
	 * 
	 * @param propertyName
	 *            property name
	 * @param value
	 *            property value which type is {@link JSONArray}
	 * @return current JSON object
	 */
